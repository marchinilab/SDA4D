...
#### Running a 2D Matrix Factorisation

To run a 2D matrix factorisation,
restructure the N by L matrix into an N by L by 1 by 1 tensor with the 
provided `matrixToTensor` function and
use RunSDA4D as above. This amounts to using arguments 
`data_tensor = matrixToTensor(data_matrix), dimn_vector = c(dim(data_matrix),1,1)`. 
Since the third and fourth dimensions 
are 1, RunSDA4D will automatically run a 2D matrix factorisation 
by fixing the time and tissue scores matrices D,B to be constant 1 by C matrices with value 1.
All iterations will neglect to update these matrices.  This is equivalent to a
